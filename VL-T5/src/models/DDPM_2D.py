...
class DDPM_2D(LightningModule):
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        AnomalyScoreReco = []
        latentSpace = []
        index_tum=torch.nonzero(data_seg.squeeze().sum(dim=(0,1)) != 0).squeeze(1)
        input=input[:,:,:,:,index_tum]
        data_orig=data_orig[:,:,:,:,index_tum]
        data_seg=data_seg[:,:,:,:,index_tum]
        data_mask=data_mask[:,:,:,:,index_tum]
        #print(input.shape,data_orig.shape,data_seg.shape,data_mask.shape)
