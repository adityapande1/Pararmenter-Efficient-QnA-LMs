...
import numpy as np
import torch
import elasticdeform.torch as elasticdeform
...
def Train(csv,cfg,preload=True):
    ...
    for _, sub in csv.iterrows():
        ...
        subject_dict['seg'] = tio.LabelMap(sub.img_path.replace("t2","seg"),reader=sitk_reader)
...
def Eval(csv,cfg): 
    ...
    for _, sub in csv.iterrows():
        ...
        subject_dict = {
            ...
            'seg_available': False,
            'path' : sub.img_path }
        if sub.setname=='Brats21':
            if sub.seg_path != None: # if we have segmentations
                subject_dict['seg'] = tio.LabelMap(sub.seg_path.replace("t2","seg"),reader=sitk_reader), #.replace("t2","seg")
                subject_dict['seg_orig'] = tio.LabelMap(sub.seg_path.replace("t2","seg"),reader=sitk_reader)# we need the image in original size for evaluation .replace("t2","seg")
                subject_dict['seg_available'] = True
        else:
            if sub.seg_path != None: # if we have segmentations             
                subject_dict['seg'] = tio.LabelMap(sub.seg_path,reader=sitk_reader), #.replace("t2","seg")
                subject_dict['seg_orig'] = tio.LabelMap(sub.seg_path,reader=sitk_reader)# we need the image in original size for evaluation .replace("t2","seg")
                subject_dict['seg_available'] = True        
            #print(sub.seg_path.replace("t2","seg"))
        if sub.mask_path != None: # if we have masks
            subject_dict['mask'] = tio.LabelMap(sub.mask_path,reader=sitk_reader)
        ...
        subjects.append(subject)
    ds = tio.SubjectsDataset(subjects, transform = get_transform(cfg))
    #print(sub.seg_path,sub.img_path)
    return ds
...
class vol2slice(Dataset):
    ...
    def __getitem__(self, index):
            ...
            #try:
             #   ind=random.randint(1, 380)
              #  subject1 = self.ds.__getitem__(ind)
               # subject['vol_fp'] = subject1['vol'].data[...,self.ind].squeeze()
                #break
           # except:
            #    subject['vol_fp'] = subject['vol'].data[...,self.ind].squeeze()
                #print(ind)
        #ind=random.randint(1, 380)
        #subject1 = self.ds.__getitem__(ind)
        #s=time.time()
        #subject['syn_tum'] = elastic_mass(subject['vol_fp'].data)
        #e=time.time()
        #print("elstic",e-s)
        count=0
        Flag=True
        ...
        #print("data shape",torch.sum(subject['seg'].data[...,self.ind]).item())
        #print("index",self.ind)
        #subject['seg'].data = subject['seg'].data[...,self.ind]
        #subject["label"]=int(torch.sum(subject['seg'].data).item()==0)
        return subject
...
def create_ellipsoid_masks(height, width, num_masks):
    masks = []
...
def elastic_mass(image):
    image=torch.zeros_like(image)
    tiny_radius = torch.randint(32,64,(1,))
    ...
    points = 3
    points = [points] * 2
    ell = create_ellipsoid_masks(x, y, 1)[0]  # Assuming create_ellipsoid_masks is a PyTorch function
    displacement = torch.randn(2, *points) * sigma
    geo = elasticdeform.deform_grid(ell,displacement, order=0, axis=(0, 1))
    x_,y_=torch.randint(0,image.shape[0]-geo.shape[0],(1,)),torch.randint(0,image.shape[1]-geo.shape[1],(1,))
    image[x_:x_+geo.shape[0],y_:y_+geo.shape[1]]=geo
    return image
