...
class IXI(LightningDataModule):
    ...
    def __init__(self, cfg, fold = None):
        ...
        for state in states:
            ...
            self.csv[state]['mask_path'] = None #cfg.path.pathBase + '/Data/' + self.csv[state]['mask_path']
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not hasattr(self,'train'):
            if self.cfg.sample_set: # for debugging
                ...
                self.classifier_train=create_dataset.Eval(self.csv['train'],self.cfg)
    
    def train_dataloader(self):
        return DataLoader(self.train, batch_size=self.cfg.batch_size,num_workers=self.cfg.num_workers, pin_memory=True, shuffle=True, drop_last=self.cfg.get('droplast',False)) #num_workers=self.cfg.num_workers

    def val_dataloader(self):
        return DataLoader(self.val, batch_size=self.cfg.batch_size, num_workers=self.cfg.num_workers, pin_memory=True, shuffle=False)  #num_workers=self.cfg.num_workers
    ...
    def classifier_dataloader(self):
        return DataLoader(self.classifier_train, batch_size=1,num_workers=self.cfg.num_workers, pin_memory=True, shuffle=True, drop_last=self.cfg.get('droplast',False))  
